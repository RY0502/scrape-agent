...
export class SambaNovaProvider implements VisionProvider {
  name = "SambaNova";
  ...
  constructor(apiKey: string, model: string, maxTokens: number, baseUrl?: string) {
    ...
    this.apiUrl = baseUrl || "https://api.sambanova.ai/v1/chat/completions";
  }
  ...
  async extract(prompt: string, screenshotBase64: string): Promise<string> {
    ...
    const data = await response.json() as SambaNovaResponse;

    if (data.error) {
      throw new Error(`SambaNova API error: ${data.error.message || "Unknown error"}`);
    }
